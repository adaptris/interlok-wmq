 ...
 * <p>
 * It is possible to encounter errors where the message read from the Websphere MQ queue cannot be translated to an AdaptrisMessage.
 * Possible causes for this include; incorrect mapping configuration or an incorrect message type configuration.
 * </p>
 * <p>
 * In these cases the message will effectively be lost. We have consumed the message from WMQ, which means the message will be
 * removed from the queue, but seeing as we cannot translate the message an exception will be thrown, but the message will be gone.
 * This error handler fixes this very niche problem. Any errors encountered, once the message has been consumed from WMQ will be
 * forwarded to this error handler.
 * </p>
 * <p>
 * The specific implementation of this error handler is to forward the MQMessage to any queue you specify in configuration. NOTE:
 * The connection details to the broker will be taken from the parent consumer. You may only specify queue name to forward error
 * messages to.
 * </p>
 * <p>
 * <code>MessageOptions</code> are required to connect to a queue and send messages. This options will be taken from the parent
